...
between three groups, one for each central carousel row. For every new shooting
session the full pool is shuffled independently. Each camera frame consumes the
next `2 × pose_examples_per_side` unused files: first the left column from top
to bottom, then the right column from top to bottom.
