...
namespace CameraControl
{
    ...
    {
        public DownloadAllFilesCommand(ref CameraModel model, ref IntPtr volume) : base(ref model) { _volume = volume; }
        ...
        private static string _targetFolderName;  // Folder to download.
        ...
        public override bool Execute()
        {
            ...
            // Loop until downloading DCIM and XFVC.
            while (true)
            {
                ...
                if (err == EDSDKLib.EDSDK.EDS_ERR_OPERATION_CANCELLED)
                {
                    return true;
                }
                else if (err != EDSDKLib.EDSDK.EDS_ERR_OK)
                {
                    return false;
                }
        ...
        private uint DownloadImageByDirectory(IntPtr camera)
        {
            ...
            for (index = 0; index < _imageItems.Count; ++index)
            {
                ...
                if (_model._ExecuteStatus == CameraModel.Status.CANCELING)
                {
                    ...
                    _currentFileNum = 0;
                    _imageItems.Clear();
                    return EDSDKLib.EDSDK.EDS_ERR_OPERATION_CANCELLED;
                }
                else
                {
